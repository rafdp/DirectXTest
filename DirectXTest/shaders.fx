			  ...
			  inout TriangleStream<PS_INPUT> output)
{
	...
	float3 upAxis = normalize (float3 (input[0].worldPos.x, 
									   ...
									   input[0].worldPos.z - normal.y / normal.z));
	...
	if (d < Range)
	{
		...
		float3 shift = float3 (rand (float2(outputVert[0].worldPos.x, outputVert[0].worldPos.y)),
							   rand (float2(outputVert[1].worldPos.y, outputVert[1].worldPos.z)),
							   rand (float2(outputVert[2].worldPos.z, outputVert[2].worldPos.x)));
